...
// The color each single-hued overlay is drawn in, in one place, because two things read it: the
...
// ground itself, and a uniform step left two of them compositing to the same color. What a pair
...
// The dot colors match the route panel's scenery sliders (landmark amber, art fuchsia), so the map
...
// dot: names are drawn in the dot's own color over a black halo (src/tiles/labels.ts), and a dark
// fill on a dark halo comes out as mud. The other POI colors are already light enough to carry
...
// The route layer's ferry-leg color as well as the ferry overlay's.
...
// uniform color — the overlay is binary on/off, with no intensity grading yet.
//
...
// shows the raw color at full strength, where a saturated pink beside two washed fields is the one
...
// The subway draws each route in the transit agency's own published color (src/tiles/subway.ts), so
// it is the one overlay with no single true color. This is the A/C/E blue the feed publishes, which
...
// lighter at night for the same reason the dot colors are, the saturated green being read against
