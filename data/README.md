...
| `highways/nyc.bin` | big roads (motorway through tertiary, ramps folded in) and above-ground rail as polylines, each with a class byte and a severity byte — the road/rail proximity *penalty*, weighted by each road's traffic count | **OpenStreetMap** + FHWA HPMS 2024 (AADT) |
